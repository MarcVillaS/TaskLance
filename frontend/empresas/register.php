...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($correo) || empty($password) || empty($password2)) {
        ...
        $result = @file_get_contents('http://localhost:3000/api/empresas/register', false, $context);
        if ($result === FALSE) {
...
<div class="registro-container">
    ...
    <form method="POST" novalidate>
        <input type="text" name="nombre" placeholder="Nombre de la empresa" maxlength="80" required autocomplete="off" value="<?= isset($nombre) ? htmlspecialchars($nombre) : '' ?>">
        <input type="email" name="correo" placeholder="Correo electrónico" maxlength="100" required autocomplete="off" value="<?= isset($correo) ? htmlspecialchars($correo) : '' ?>">
