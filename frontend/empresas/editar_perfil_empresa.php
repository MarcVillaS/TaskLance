...
if (!isset($_SESSION['empresa_id'])) {
    header("Location: login_empresas.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $permitidas)) {
            $dest = 'img/avatars/empresa_' . $empresa_id . '.' . $ext;
    ...
    if (!$error) {
        $stmt = $pdo->prepare("UPDATE empresas SET nombre=?, correo=?, avatar=? WHERE id=?");
        $stmt->execute([$nombre, $correo, $avatar, $empresa_id]);
        $mensaje = "¡Perfil actualizado!";
        $empresa = ['nombre'=>$nombre, 'correo'=>$correo, 'avatar'=>$avatar];

        if ($cambiar_contra) {
            $hash_nuevo = password_hash($password_nueva, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("UPDATE empresas SET password=? WHERE id=?");
            $stmt->execute([$hash_nuevo, $empresa_id]);
...
$avatar_path = $empresa['avatar'] ?: 'img/default-avatar.png';
...
<a href="inicio_empresas.php" class="back-link">&larr; Volver</a>
...
<div class="form-perfil">
    ...
    <form method="post" enctype="multipart/form-data" autocomplete="off">
        ...
        <input type="text" name="nombre" value="<?= htmlspecialchars($empresa['nombre']) ?>" required maxlength="100" />

        <label>Email:</label>
        <input type="email" name="correo" value="<?= htmlspecialchars($empresa['correo']) ?>" required maxlength="100" />
